...
static void serve_static(int fd, char *filename, size_t filesize, hs_http_out_t *out);

static char *ROOT = NULL;

mime_type_t hspeed_mime[] = 
{
    {".html", "text/html"},
    {".xml", "text/xml"},
...
const char* get_file_type(const char *type){
    ...
    for(int i = 0; hspeed_mime[i].type != NULL; ++i){
        if(strcmp(type, hspeed_mime[i].type) == 0)
            return hspeed_mime[i].value;
    }
    // 未识别返回"text/plain"
    return "text/plain";
}
...
void do_error(int fd, char *cause, char *err_num, char *short_msg, char *long_msg){
    ...
    sprintf(body, "%s<hr><em>HSpeed web server</em>\n</body></html>", body);

    // 返回错误码，组织错误响应头
    sprintf(header, "HTTP/1.1 %s %s\r\n", err_num, short_msg);
    sprintf(header, "%sServer: HSpeed\r\n", header);
...
// 处理静态文件请求
void serve_static(int fd, char *filename, size_t filesize, hs_http_out_t *out){
    // 响应头缓冲（512字节）和数据缓冲（8192字节）
    char header[MAXLINE];
    char buff[SHORTLINE];
    struct tm tm;
...
int error_proess(struct stat* sbufptr, char *filename, int fd){
    ...
    if(stat(filename, sbufptr) < 0){
        do_error(fd, filename, "404", "Not Found", "HSpeed can't find the file");
        return 1;
    }

    // 处理权限错误
    if(!(S_ISREG((*sbufptr).st_mode)) || !(S_IRUSR & (*sbufptr).st_mode)){
        do_error(fd, filename, "403", "Forbidden", "HSpeed can't read the file");
        return 1;
    }
...
void do_request(void* ptr){
    hs_http_request_t* request = (hs_http_request_t*)ptr;
    int fd = request->fd;
    ROOT = request->root;
    ...
    while(1){
        ...
        // 解析请求报文行
        rc = hs_http_parse_request_line(request);
        if(rc == HS_AGAIN)
            continue;
        else if(rc != 0)
            goto err;
        ...
        // 解析请求报文体
        rc = hs_http_parse_request_body(request);
        if(rc == HS_AGAIN)
            continue;
        else if(rc != 0)
            goto err;
        ...
        // 分配并初始化返回(响应)数据结构
        hs_http_out_t* out = (hs_http_out_t *)malloc(sizeof(hs_http_out_t));
        hs_init_out_t(out, fd);
        ...
        hs_http_handle_header(request, out);
    ...
    // 修改已经注册描述符的事件类型
    // 重置定时器，每等待下一次请求均生效
    hs_epoll_mod(request->epoll_fd, request->fd, request, (EPOLLIN | EPOLLET | EPOLLONESHOT));
    hs_add_timer(request, TIMEOUT_DEFAULT, hs_http_close_conn);
