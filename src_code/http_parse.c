        ...
        switch(state){
            case sw_start:
                ...
                if(ch == CR || ch == LF)
                    break;
                if((ch < 'A' || ch > 'Z') && ch != '_')
                    return HS_HTTP_PARSE_INVALID_METHOD;
                state = sw_method;
                break;
            ...
            case sw_method:
                if(ch == ' '){
                    ...
                    switch(p - m){
                        case 3:
                            if(hs_str3_cmp(m, 'G', 'E', 'T', ' ')){
                                request->method = HS_HTTP_GET;
                                break;
                            }
                            break;
                        case 4:
                            if(hs_str3_cmp(m, 'P', 'O', 'S', 'T')){
                                request->method = HS_HTTP_POST;
                                break;
                            }
                            if(hs_str3_cmp(m, 'H', 'E', 'A', 'D')){
                                request->method = HS_HTTP_HEAD;
                                break;
                            }
                            break;
                        default:
                            request->method = HS_HTTP_UNKNOWN;
                            break;
                    }
            ...
            case sw_spaces_before_uri:
                ...
                switch(ch){
                    case ' ':
                        break;
                    default:
                        return HS_HTTP_PARSE_INVALID_REQUEST;
                }
                break;
        ...
        switch(state){
            ...
            case sw_spaces_before_colon:
                if(ch == ' ')
                    ...
                    return HS_HTTP_PARSE_INVALID_HEADER;
            ...
            case sw_cr:
                if(ch == LF){
                    state = sw_crlf;
                    hd = (hs_http_header_t *) malloc(sizeof(hs_http_header_t));
