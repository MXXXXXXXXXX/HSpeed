...
void accept_connection(int listen_fd, int epoll_fd, char* path) {
	...
	while (1) {
		...
		// 申请hs_http_request_t类型节点并初始化
		hs_http_request_t* request = (hs_http_request_t*)malloc(sizeof(hs_http_request_t));
		hs_init_request_t(request, accept_fd, epoll_fd, path);

		// 文件描述符可以读，边缘触发(Edge Triggered)模式，保证一个socket连接在任一时刻只被一个线程处理
		hs_epoll_add(epoll_fd, accept_fd, request, (EPOLLIN | EPOLLET | EPOLLONESHOT));
