...
static int threadpool_free(hs_threadpool_t *pool);
static void* threadpool_worker(void *arg);

// 释放线程池
int threadpool_free(hs_threadpool_t *pool){
...
// 释放线程资源
int threadpool_destory(hs_threadpool_t *pool, int graceful){
    if(pool == NULL)
        return hs_tp_invalid;
    if(pthread_mutex_lock(&(pool->lock)) != 0)
        return hs_tp_lock_fail;

    int err = 0;
    do{
        if(pool->shutdown){
            err = hs_tp_already_shutdown;
            break;
        }
...
hs_threadpool_t *threadpool_init(int thread_num){
    ...
    // 分配并初始化task头结点
    pool->head = (hs_task_t*)malloc(sizeof(hs_task_t));
